...
export const GET: RequestHandler = async ({ url, cookies }) => {
	...
	try {
		...
		if (filterType === 'album' && data.albums?.items && Array.isArray(data.albums.items)) {
			...
			// Convert to our deduplication format
			const spotifyAlbums: SpotifyAlbumObjectSimplified[] = rawAlbums.map((album: any) => ({
				id: album.id,
				name: album.name,
			...
			try {
				...
				console.warn('Album deduplication failed, falling back to raw results:', dedupError);
				// Fallback: add original albums without deduplication
		...
		if (filterType === 'playlist' && data.playlists?.items && Array.isArray(data.playlists.items)) {
			data.playlists.items
				.filter(
					(playlist: any) =>
						playlist && playlist.id && playlist.name && playlist.tracks && playlist.tracks.total > 0
				)
			...
			userPlaylistsData.items
				.filter((playlist: any) => {
					...
					const descriptionMatches =
						playlist.description?.toLowerCase().includes(queryLower) || false;
				...
				.forEach((playlist: any) => {
					...
					const alreadyExists = results.some(
						(result) => result.type === 'playlist' && result.id === playlist.id
					);
					...
					if (!alreadyExists) {
						...
						const existingPlaylist = results.find(
							(result) => result.type === 'playlist' && result.id === playlist.id
						);
						if (existingPlaylist) {
