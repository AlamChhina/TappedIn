<script lang="ts">
	...
	function ensureSession() {
		...
		gameHistory.subscribe((history) => {
			const shouldStart = gameHistory.shouldStartNewSession(
				currentSessionId,
				'classic', // This is the classic mode component
				playbackMode,
				item.id,
				history
			);
	...
	async function initializePlayer() {
		try {
			...
			// Prime mobile connection BEFORE creating the main player
			if (isMobileDevice()) {
				console.log('📱 Priming mobile connection...');
				const result = await fastMobileConnectionManager.primeMobileConnection();
				if (result.success) {
					console.log('✅ Mobile connection primed successfully');
				} else {
					console.warn('⚠️ Mobile priming failed, will use fallback:', result.message);
				}
			}
			...
			player.addListener('player_state_changed', (state: any) => {
				if (state) {
					...
					if (!state.paused && state.position > 0 && playbackStartTime === null && !isPlayingFullSong) {
						...
						targetStopPosition = startPosition + (duration * 1000); // Start position + duration in milliseconds
						...
						timingDebugInfo = `Started at ${startPosition}ms, target: ${targetStopPosition}ms`;
	...
	async function playFromStart() {
		...
		try {
			...
			const duration = getCurrentDuration();
			const startPosition = playbackMode === 'random' ? randomStartPosition : 0;
			console.log(`🎵 Starting enhanced mobile playback (Classic Mode - ${duration} seconds)`);
	...
	async function playFullSong() {
		...
		try {
			...
			// Use same approach as track snippet playback
			if (isMobileDevice() && fastMobileConnectionManager.isPrimed()) {
				console.log('📱 Using primed connection for full song');
			}
			...
			if (!response.ok) {
				...
				// Same fallback logic as snippet playback
				if (response.status === 404 && isMobileDevice()) {
					console.log('📱 Full song: Primed connection failed, falling back...');
					try {
						await mobileSpotifyManager.activateDevice(deviceId);
						const retryResponse = await fetch(`/api/spotify/player/play?device_id=${deviceId}`, {
							method: 'PUT',
						...
						throw new Error(`Full song failed: ${errorText}. Activation failed: ${activationError instanceof Error ? activationError.message : 'Unknown error'}`);
					}
				} else {
					throw new Error(errorText);
				}
	...
	function submitGuess() {
		...
		// Track the guess in history with classic mode details
		if (currentSessionId && currentTrack) {
			gameHistory.addGuess(
				...
				isCorrect,
				triesUsed, // Number of tries used for classic mode
				maxTries   // Max tries available
			);
		}
			...
			<div class="mb-4 flex items-center justify-between">
				...
				<div class="relative {showAnswer ? 'mb-4' : ''}">
					<input
						...
						placeholder={isFirstSongForArtist
							? 'Press play to hear the first second, then guess!'
							: 'Enter your guess...'}
				...
				{#if showAnswer}
					<div
						...
						style="border-color: {guessStatus === 'correct' ? '#059669' : '#dc2626'}; background-color: {guessStatus === 'correct' ? 'rgba(5, 150, 105, 0.1)' : 'rgba(220, 38, 38, 0.1)'};"
					>
						<div class="flex items-center justify-between">
							<div>
